...
| Capability fit, not capability hoarding | Skills, MCP servers, hooks, subagents, and external research are judged as separate coverage rows and recommended only when they close a real gap in the task or repo. Capability Discovery now produces a short signal-bound recommendation table: repo signal, coverage row, why, install surface, risk/cost, fallback, and classification. The bundled `find-skills` helper is for skill discovery; web research is used for current hooks, MCP, and tool behavior. |
