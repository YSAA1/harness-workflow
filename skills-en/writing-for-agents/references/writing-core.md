...
> Upstream: [mattpocock/skills](https://github.com/mattpocock/skills) `writing-for-agents` (MIT), pinned at revision `f054def` (2026-07-31 snapshot; upstream has since only done an em-dash punctuation cleanup `3216582`). English body verbatim from that revision; the mechanics pointer is retargeted to `skill-mechanics.md`, and one clarifying word added ("what follows" → "what follows it").
...
- **By sequence** — split a run of steps where the post-completion steps tempt the agent to rush the one in front of it. Keeping them out of view drives more legwork on the current task. Beware the reverse: merging sequences exposes each step's later steps to what follows it, inviting premature completion.
