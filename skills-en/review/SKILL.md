...
7. Persist findings (conditionally): all findings handled on the spot → do not persist to disk. If there are findings pending across sessions and the project has a recovery surface → write to this lane's state surface: state.md Evidence records the findings list (number/location/severity) and the ready verdict, and Next records the pending fixes; when findings are numerous or need independent tracking, persist to `docs/reviews/YYYY-MM-DD--<topic>-review.md`, linked from this lane's row or from state. Projects without a recovery surface keep in-session handoff and create no new state files.
