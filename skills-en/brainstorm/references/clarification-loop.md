...
| File | Role |
| --- | --- |
| This file | Frontier loop, priority, and Grill Gate (single source of the gate) |
...
6. **Capability gaps** — if `unknown` or unexamined; repair suggestions wait until after Spec approval — do not hijack the frontier
...
3. On non-trivial tasks, no blocking design branch is left unexamined: every Branch Order branch is answered, explicitly waived, or repo-provable; there is no minimum number of questioning rounds.
