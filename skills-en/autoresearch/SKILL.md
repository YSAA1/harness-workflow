...
When companion skills (`plan`/`implement`/`review`/`cleanup`) or the recovery surface are missing, self-check and self-close in the spirit of their protocols: without a recovery surface, the single log file is itself the lane and `.harness/` is not force-created — default path `docs/research/YYYY-MM-DD--<topic>.md` (follow the project's research/notes conventions if present); the Track line in the log header self-registers the lane, and on re-entering this skill first search existing logs under `docs/research/`; without a lessons surface, distilled entries are recorded in a final section of the log. Hand off to `brainstorm` only when the research question itself is a user design trade-off that blocks further progress; once handed off, this skill ends.
...
Before the first round: read lessons and existing logs; if a recovery surface exists, `plan` establishes the lane (a work_index row + the log). Each round has four segments:
...
2. **Gather evidence**: execute reading/searching/experiments directly; route code changes through `implement` (experiment subtasks stay independent and do not touch the main research lane); when the environment has read-only delegation capabilities such as subagents or cli-delegate, bulk reading/searching may be delegated to run in the background while the main loop holds only the hypothesis and the verdict — delegated results are still claims, and key observations are persisted to the log only after their sources and acquisition times have been checked back in the main session. Done when: observations land in the log together with their sources and acquisition times.
