...
2. 落点分层：
   - 完整交接文档一律写到操作系统临时目录（如 /tmp；不进仓库或项目目录，避免制造孤儿文件），把路径明确告诉用户。
   - 项目有恢复面：续点优先写进本轨道 state 的 Next 行，关键状态与证据写 Evidence 行（持久、可恢复）；完整交接文档仅在确需完整上下文时才生成。
   - 无恢复面：临时目录里的交接文档就是唯一交接物。
3. 内容纪律：
