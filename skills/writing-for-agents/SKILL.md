...
2. 按 `references/recovery_surface_policy.md` 选最小 backend（none / lightweight / harness / feature-list / existing）；每个任务/轨道一个权威入口，独立轨道可同时 active。新任务的 harness 最小面（work_index + state）由 `plan` 内联建立（`brainstorm` 在已有恢复面项目仅预登记本轨道行，不建面）；补充文件模板在 `../harness-builder/templates/`（work_index/state/lessons），按需实例化；未安装 harness-builder 时按 `references/recovery_surface_policy.md` 的可恢复字段集手写同构文件，不依赖模板存在。
