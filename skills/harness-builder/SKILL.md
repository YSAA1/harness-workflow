...
Build or repair the minimal useful **project-level harness** for AI coding agents: project map, Project iron laws, protected paths, verification entry, selected recovery surface under `.harness/` (`recovery_policy.md`, `work_index.md`, `state.md`, `progress.md`, `decisions.md`), reusable skills, hooks, MCP policy, subagent policy, audit records, and cleanup discipline. "bootstrap" is only a historical alias.
...
4. **Recommendation table** — One `Required / Recommended / Deferred / Rejected` table across: agent entry and project map; static docs and durable rules; recovery surface; **recovery policy and work index**; verification entry; architecture boundaries; anti-entropy; skills; hooks; MCP; subagents; plugins; commands/CI/headless automation; external research; dynamic context; commit protocol (default `Deferred`). See `references/recommendation_matrix_policy.md`, `references/recovery_policy.md`, `references/source_of_truth_tiers.md`, `references/living_docs_discipline.md`, `references/architecture_enforcement_policy.md`, `references/anti_entropy.md`, and `references/automation_recommendation_guide.md`.
