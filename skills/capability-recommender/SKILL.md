...
Adapted from Anthropic official `claude-automation-recommender`. The main workflow, categories, tables, and reference routing below are copied from the official skill, with harness-workflow notes added for Codex, Claude Code, and Cursor.

Analyze codebase patterns to recommend tailored agent workbench capabilities across all extensibility options.
...
- Keep the official recommendation flow intact, but map outputs to harness-workflow surfaces when relevant: Codex skills, Claude Code skills/plugins, Cursor rules/skills, repository scripts, and verification/recovery surfaces.
- Do not install hooks, MCP servers, subagents, or global config by default; recommend them with cost, risk, and rollback notes.
- For durable recovery or project memory gaps, route to `recovery-surface-builder`. For AGENTS/CLAUDE/Cursor instruction changes, route to `agent-instructions-maintainer`.
