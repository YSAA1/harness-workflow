...
一个关键点：`plan` 默认写到 `docs/plans/YYYY-MM-DD--<topic>-plan.md`，不强行创建 `task_plan.md`、`progress.md`、`findings.md`。运行时恢复默认走 `.harness/`，也可以复用 issue、feature list 或项目已有系统；遇到旧 root 三文件时只迁移或归档，不再提供模板。
