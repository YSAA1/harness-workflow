...
1. **Frontier grill**: it lists every unresolved material question for you in one numbered batch — each with a recommended answer, so you just confirm or amend item by item instead of composing essays. Questions cover goal, scope, trade-offs, success criteria and verification. What is already settled is not re-asked, and "industry convention" never decides for you.
2. **Spec drafting**: once the questions hit zero, it drafts a Spec for your review. Short needs take the lightweight Thin Spec — one round of questions, not a thesis defense.
