...
- skills.sh 目录页是批量/爬取节奏的快照而非实时（2026-09-23 实测：全量 CLI 安装后 ≥5 分钟页面零变化）：新技能发布后跑一次 `npx skills add` 触发遥测，**隔日**再复查仓库页收录，安装当天页面不变不代表收录失败；收录机制最可能与安装事件绑定但批量生效（任务 027 两轮 inconclusive）。
...
- 循环式协议的重开/停止条件由执行者自评会被双向架空——既可每轮自我论证「新证据」永续，也可窄假设一轮假收敛：重开判定须交对抗审查并诚实标注 independent/self，同体填表不构成第三方复核（2026-09-21 Grok+Codex 两轮对抗实证，autoresearch 证据门三轮演进）；brainstorm grill 判空同构（2026-09-24 用户实测一轮泛问不贴设计），修法按任务价值比例选：轻配=判空宣告须附八支一行对账显式化，重配=对抗复核。
...
- 双语两树是条款级漂移高发面：改中文树任何技能文件必须同 commit 同步 skills-en/ 对应文件——check-plugin 等式闸门只拦目录缺失/坏 name/整树缺席，拦不住条款内容漂移，遗漏靠审校轮兜底（2026-09-23 任务 024 审校实证 F1-F5 四类缺口，全部修于发布前）。
