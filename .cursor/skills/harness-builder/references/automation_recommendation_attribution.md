...
- Added Harness coverage-row binding, approval boundary, trust boundary, fallback, verification probe, and classification fields.
- Kept the `harness-builder` runtime entry light and moved long tables into references.
- Preserved recommendation-only behavior by default; installation still requires an approved Harness Plan or explicit user approval depending on surface.
