...
**Hard rules:** Do not execute the supplied task here. Reconcile existing harness before adding/replacing. Audit `AGENTS.md` every run; patch only stable facts. Dynamic state → recovery surface only (`active_slice`, plans, session notes, Research Route state). Hot recovery docs are bounded indexes, not append-only reports; roll up old progress into links, decisions, and residual risks. Status/check/selftest scripts are views/probes, not state stores: they may print pointers and validate files, but must not mirror active slices, evidence logs, probe inventories, or long conclusions. Do not create a second recovery surface. Add files or capabilities only when they close a named coverage gap. Keep `AGENTS.md` thin. Prefer mechanical enforcement (tests, lint, ratchets) over prose-only rules. Capability decisions cover skills, hooks, MCP, subagents, external research, CI, GC.
