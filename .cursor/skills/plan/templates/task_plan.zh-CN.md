...
- 当前切片（Active slice）：[当前唯一推进的最小可验证切片]
- 非目标（Non-goals）：[本轮明确不做的事情]
- 成功标准（Success criteria）：[如何判断 active slice 完成]
- 验证路径（Verification path）：[需要运行的命令、smoke / E2E 或人工信号]
- 验证路径状态（Verification path status）：`runnable | blocked`
- 所需能力（Required capabilities）：[验证所需脚本、服务、浏览器、MCP、人工检查等]
- 替代证据（Fallback evidence）：[完整验证不可用时，用户接受的替代证据；没有则写 `none`]
- 最终集成声明（Final integration claim）：[多阶段或多 commit unit 的最终整体验收声明；不适用写 `none`]
- 项目地图（Project map）：[项目地图位置，例如 `docs/project-map.md` 或 `AGENTS.md#项目地图`]
...
状态：`in_progress`
验收标准（acceptance_criteria）：[可证伪的完成条件]
验证命令（verification_commands）：[`命令1`, `命令2`]
